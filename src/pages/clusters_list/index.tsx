            ...
            <HomePageMainContainer>
                ...
                <ClusterCirclesGridContainer>
                    {clusters.map((item:any, index:any) => (
                        ...
                        <LinkNoStyled key={index} style={{textDecoration: 'none', color: '#ffffff'}} href={`/cluster_info/${item.name}`}>
                            <ClusterCircleElement key={index}  className = {item.glowed ? 'glowed' : ''}  size={randomSize[index]} style={{  margin: randomMar[index] + `px` }}>{item.name}</ClusterCircleElement>
                        </LinkNoStyled>
